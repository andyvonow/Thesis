...
%                         hold off
    ...
    thr = fitlm([MasterVars.ShankN3.(AccelVar)],[MasterVars.ShankN3.(ForceVar)]);
    
    timethr = linspace(min([MasterVars.ShankN3.(AccelVar)]),max([MasterVars.ShankN3.(AccelVar)]));
...
%         ["Global Fit"+newline+"R^2 = "+num2str(round(thr.Rsquared.Ordinary,2))]],'Location','northwest');
    ...
    OrigAccel = [MasterVars.("P"+num2str(NCV)+"Shank").(AccelVar)];
    OrigForce = [MasterVars.("P"+num2str(NCV)+"Shank").(ForceVar)];
...
%     title("Validation: Shank"); xlabel(yLab); ylabel(xLab);
%     legend(["P"+num2str(NCV)+" Actual"],["P"+num2str(NCV)+" Predicted"],'Location','northwest');
    
    P3ShankFitLm = fitlm(OrigForce,P3Predicted);
    xAxisForceFit = linspace(min(OrigForce),max(OrigForce));
    yAxisForceFit = P3ShankFitLm.Coefficients.Estimate(1) + P3ShankFitLm.Coefficients.Estimate(2) .* xAxisForceFit;
    
%     figure(7); clf;
%     hold on
%     plot(OrigForce, P3Predicted,'ko');
%     plot(xAxisForceFit, yAxisForceFit,'m-');
%     title("Validation: Shank"); xlabel("Actual: " + xLab); ylabel("Predicted: " + xLab);
%     legend(["P4 Predicted"+newline+...
%         "R^2 = " + num2str(round(P3ShankFitLm.Rsquared.Ordinary,2))],'Location','northwest');
    

    writecell({["NCV("+NCV+")_P"+num2str(NCV)+"Shank"],...
        [ForceVar{1}(7:end)+" - "+AccelVar{1}(7:end)],[P3ShankFitLm.RMSE],...
        [thisNCV.RMSE]},...
