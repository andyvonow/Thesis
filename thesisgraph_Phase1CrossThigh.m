            ...
            for n=1:NumVar %columns
                ...
                if (n < m) && ...
                    ...
                    if contains(string(MasterFields(MasterVar)), "P") && ...
                        ...
                        if contains(string(MasterFields(MasterVar)),"Thigh")
                            figureNumber = 3;
                            plotting = true;
...
%                         hold off
    ...
    thr = fitlm([MasterVars.ThighN3.(AccelVar)],[MasterVars.ThighN3.(ForceVar)]);
    
    timethr = linspace(min([MasterVars.ThighN3.(AccelVar)]),max([MasterVars.ThighN3.(AccelVar)]));
...
%         ["Global Fit"+newline+"R^2 = "+num2str(round(thr.Rsquared.Ordinary,2))]],'Location','northwest');
    ...
    OrigAccel = [MasterVars.("P"+num2str(NCV)+"Thigh").(AccelVar)];
    OrigForce = [MasterVars.("P"+num2str(NCV)+"Thigh").(ForceVar)];
...
%     title("Validation: Thigh"); xlabel(yLab); ylabel(xLab);
%     legend(["P"+num2str(NCV)+" Actual"],["P"+num2str(NCV)+" Predicted"],'Location','northwest');
    
    P3ThighFitLm = fitlm(OrigForce,P3Predicted);
    xAxisForceFit = linspace(min(OrigForce),max(OrigForce));
    yAxisForceFit = P3ThighFitLm.Coefficients.Estimate(1) + P3ThighFitLm.Coefficients.Estimate(2) .* xAxisForceFit;
    
%     figure(7); clf;
%     hold on
%     plot(OrigForce, P3Predicted,'ko');
%     plot(xAxisForceFit, yAxisForceFit,'m-');
%     title("Validation: Thigh"); xlabel("Actual: " + xLab); ylabel("Predicted: " + xLab);
%     legend(["P4 Predicted"+newline+...
%         "R^2 = " + num2str(round(P3ThighFitLm.Rsquared.Ordinary,2))],'Location','northwest');
    

    writecell({["NCV("+NCV+")_P"+num2str(NCV)+"Thigh"],...
        [ForceVar{1}(7:end)+" - "+AccelVar{1}(7:end)],[P3ThighFitLm.RMSE],...
        [thisNCV.RMSE]},...
